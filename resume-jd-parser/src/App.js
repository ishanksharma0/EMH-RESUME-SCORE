    ...
    return Object.entries(data).map(([key, value]) => {
      ...
      return (
        <div key={key} className="section">
          {/* Work Experience & Education are collapsible */}
          {Array.isArray(value) && value.length > 0 && typeof value[0] === 'object' ? (
            <>
              <button className="collapsible" onClick={() => toggleSection(`${parentKey}${key}`)}>
                {label} {isExpanded ? '▲' : '▼'}
              </button>
              {isExpanded && (
                <div className="content">
                  {value.map((item, index) => (
                    <div key={index} className="nested-box">
                      <button className="collapsible" onClick={() => toggleSection(`${parentKey}${key}[${index}]`)}>
                        {label} {index + 1} {expandedSections[`${parentKey}${key}[${index}]`] ? '▲' : '▼'}
                      </button>
                      {expandedSections[`${parentKey}${key}[${index}]`] && renderJsonData(item, `${parentKey}${key}[${index}].`)}
                    </div>
                  ))}
                </div>
              )}
            ...
            <>
              ...
              {isExpanded && <div className="content">{renderJsonData(value, `${parentKey}${key}.`)}</div>}
            </>
          ) : (
            <div className="subsection">
              <h4>{label}</h4>
              <input type="text" className="input-box" defaultValue={value} />
            </div>
          )}
        </div>
      );
    ...
    <div className="container-fluid d-flex h-100">
        ...
        {selectedApi ? (
          <div className="upload-section">
            ...
            <div className="drop-zone">
              <input type="file" multiple={selectedApi === 'score-resumes'} onChange={handleFileUpload} />
      ...
      <aside className="results">
        ...
        <div className="scrollable-results">
          {parsedData && <div className="data-container">{renderJsonData(parsedData)}</div>}
        </div>
      </aside>
    </div>
  );
